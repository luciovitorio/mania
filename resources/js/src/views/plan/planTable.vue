<template>
    ...
    <div class="panel pb-0">
      ...
      <div class="table-responsive">
        <table class="table-hover">
          <thead>
            <tr>
              ...
              <TableHeaderCell
                @click="sortPlan"
                class="border-b-2 p-2 text-left"
                field="additionalSimplePieceValue"
                :sort-field="sortField"
                :sort-direction="sortDirection"
                >R$ Add Simples
              </TableHeaderCell>
              <TableHeaderCell
                @click="sortPlan"
                class="border-b-2 p-2 text-left"
                field="additionalDifficultPieceValue"
                :sort-field="sortField"
                :sort-direction="sortDirection"
                >R$ Add Difícil
              </TableHeaderCell>
          ...
          <tbody v-if="planStore.$state.isLoading || isLoading">
            <tr>
              <td colspan="11">
                <Spinner
          ...
          <tbody v-else-if="planStore.$state.plan.data.length === 0">
            <tr>
              <td colspan="11">
                <div
          ...
          <tbody v-else>
            <tr v-for="plan in planList" :key="plan.id">
              <td class="border-b p-2">{{ plan.name }}</td>
              <td
                class="border-b p-2 max-w-[200px] whitespace-nowrap overflow-hidden text-ellipsis"
              >
                {{ plan.pieceQuantity }}
              </td>
              <td class="border-b p-2">{{ plan.simplePieceQuantity }}</td>
              <td class="border-b p-2">{{ plan.difficultPieceQuantity }}</td>
              <td class="border-b p-2">{{ plan.simplePieceValue }}</td>
              <td class="border-b p-2">{{ plan.difficultPieceValue }}</td>
              <td class="border-b p-2">
                {{ plan.additionalSimplePieceValue }}
              </td>
              <td class="border-b p-2">
                {{ plan.additionalDifficultPieceValue }}
              </td>
              <td class="border-b p-2">
                <span :class="getStatusClass(plan.isActive)">{{
                  plan.isActive
                }}</span>
              </td>
              <td class="border-b p-2">
      ...
      <div
        v-show="!planStore.$state.isLoading"
        class="flex justify-between items-center mt-5"
      >
        <nav
          v-if="planStore.$state.plan.total > planStore.$state.plan.limit"
          class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px"
          aria-label="Pagination"
        >
          <a
            v-for="(link, i) of planStore.$state.plan.links"
            :key="i"
            :disabled="!link.url"
            href="#"
            ...
            :class="[
              ...
              i === 0 ? 'rounded-l-md' : '',
              i === planStore.$state.plan.links.length - 1
                ? 'rounded-r-md'
                : '',
  ...
  <PlanModal v-model="showModal" :plan="planModel" @close="onModalClose" />
</template>
...
const planModel = ref({ ...DEFAULT_PLAN_OBJECT });
...
function onModalClose() {
  planModel.value = { ...DEFAULT_PLAN_OBJECT };
}
